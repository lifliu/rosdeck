    ...
    async def _execute_cb(self, goal_handle):
        ...
        while rclpy.ok():
            ...
            if m is not None and m.is_terminal:
                break
            # ServerGoalHandle.is_cancel_requested is a METHOD (a bare attribute read
# would be a truthy bound method and cancel every mission).
    ...
    def shutdown(self):
        ...
        release leases, cancel in-flight goals, drop segment state.
        Checkpoint workers are daemon threads; they notice the terminal
        state and exit on their next abort check."""
        try:
